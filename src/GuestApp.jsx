...
export default function GuestApp({ currentUser, logout }) {
  ...
  // ИСПРАВЛЕНО: Теперь они берут данные из data.js, а не пустоту {}
  ...
  useEffect(() => {
    const sync = (e) => {
      try { if (e.newValue) { const parsed = JSON.parse(e.newValue); if (e.key === 'amina_orders_v11') setOrders(parsed || []); if (e.key === 'amina_tables_v11') setTables(parsed || []); if (e.key === 'amina_menu_v11') setMenu(parsed || []); } } catch (err) {}
    };
    ...
    if (pendingOrderId && (paymentStatus === 'processing' || paymentStatus === 'waiter_pending')) {
      ...
      if (checkOrder) {
        if (checkOrder.status === 'new') { setPaymentStatus('success'); setPendingOrderId(null); } 
        else if (checkOrder.status === 'rejected') { setPaymentStatus('rejected'); setPendingOrderId(null); }
      }
    }

    if (pendingOrderId && paymentStatus === 'waiter_pending') {
      const checkOrder = (orders || []).find(o => o.id === pendingOrderId);
      if (checkOrder && checkOrder.status === 'new') {
        setPaymentStatus('cash_success'); setPendingOrderId(null);
      }
    }
  ...
  let activeTableName = orderType === 'delivery' ? "Доставка" : (activeTable ? activeTable.name : "Навынос");
  ...
  const createOrderObject = (statusToSet, assignedWaiterPhone = null, assignedWaiterName = null) => {
  ...
  const confirmTransfer = () => {
    const newOrder = createOrderObject('transfer_pending');
  ...
  const handleCashSelection = () => {
    if (orderType !== 'in_hall' || !activeTable) {
      return alert('Оплата наличными доступна только при заказе за столиком в заведении!');
    }
    setPaymentStatus('select_waiter');
  };

  const confirmCashWithWaiter = (waiterPhone, waiterName) => {
    const newOrder = createOrderObject('waiter_pending', waiterPhone, waiterName);
    setOrders(prev => [newOrder, ...(prev || [])]);
    if (isPreOrderFlow && preOrderTableId) setTables(prev => (prev || []).map(t => t.id === preOrderTableId ? { ...t, status: 'occupied', bookedBy: currentUser.phone, bookedTime: bookingTime } : t));
    setPendingOrderId(newOrder.id);
  ...
  const selectRandomWaiter = () => {
    ...
    if (activeWaiters.length === 0) return alert('К сожалению, сейчас нет свободных официантов.');
  ...
  const getWaiterRating = (phone) => {
     ...
     if (wRevs.length === 0) return 'Новенький';
  ...
  return (
    <div className="app-wrapper">
      ...
      {/* --- МОДАЛЬНОЕ ОКНО ОТЗЫВА (Через 22 минуты) --- */}
      {reviewOrder && (
      ...
      {paymentStatus !== 'idle' && (
        <div className="payment-overlay">
          <div className="payment-modal">
            ...
            {/* ЭКРАН 1.5: ВЫБОР ОФИЦИАНТА (Для наличных) */}
      ...
      {activeGuestTab !== 'cart' && totalItemsCount > 0 && (
        <div style={{position: 'fixed', bottom: '85px', left: '50%', transform: 'translateX(-50%)', width: '90%', maxWidth: '400px', zIndex: 99}}>
           <button onClick={() => setActiveGuestTab('cart')} style={{width: '100%', padding: '16px 20px', background: '#111827', color: '#fff', borderRadius: '18px', border: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontWeight: '900', fontSize: '16px', cursor: 'pointer', boxShadow: '0 8px 25px rgba(17,24,39,0.3)'}}>
              <span style={{display: 'flex', alignItems: 'center', gap: '10px'}}><span style={{background: '#ea580c', padding: '4px 10px', borderRadius: '10px', color: '#fff'}}>{totalItemsCount}</span> В корзине</span>
      ...
      <div className={`categories-sidebar ${isMenuOpen ? 'open' : ''}`}>
        <button className="close-sidebar-btn" onClick={() => setIsMenuOpen(false)}><span>📂 Категории</span> <span>✕</span></button>
      ...
      <header className="top-header">
        <div className="logo-section"><h1 className="logo-title">АМИНА<span style={{ color: '#ea580c' }}>.</span></h1>
          <div className="desktop-nav">
            <button className={activeGuestTab === 'menu' ? 'active' : ''} onClick={() => setActiveGuestTab('menu')}>Меню</button>
            <button className={activeGuestTab === 'table' ? 'active' : ''} onClick={() => setActiveGuestTab('table')}>Залы</button>
            <button className={activeGuestTab === 'cart' ? 'active' : ''} onClick={() => setActiveGuestTab('cart')}>Корзина {totalItemsCount > 0 && <span style={{background: '#ea580c', color: '#fff', padding: '2px 8px', borderRadius: '12px', fontSize: '12px', marginLeft: '5px'}}>{totalItemsCount}</span>}</button>
          </div>
        </div>
        <button onClick={() => setActiveGuestTab('profile')} style={{ background: '#d1fae5', border: 'none', padding: '8px 15px', borderRadius: '12px', color: '#065f46', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer' }}>👤 {currentUser.isAnonymous ? 'Войти' : currentUser.name}</button>
      </header>
      ...
      <main className="main-content">
        {activeGuestTab === 'menu' && (
          <div className="menu-layout">
            <div className="desktop-sidebar">
              <h3 style={{margin: '0 0 10px 0', fontSize: '16px', color: 'var(--text)'}}>Категории</h3>
            ...
            <div className="content-area">
              <button className="hamburger-menu-trigger" onClick={() => setIsMenuOpen(true)}><span>☰</span> Категории меню</button>
        ...
        {activeGuestTab === 'table' && (
          <div className="tables-wrapper">
            <div className="tables-filter-bar">{tableGroupsList.map(group => (<button key={group} onClick={() => setSelectedTableGroup(group)} className={`filter-btn ${selectedTableGroup === group ? 'active' : ''}`}>{group === 'all' ? 'Все залы' : group}</button>))}</div>
        ...
        {activeGuestTab === 'cart' && (
          <div className="page-container">
            <h2 style={{marginTop: 0, marginBottom: '20px', color: '#111827'}}>Оформление заказа</h2>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', backgroundColor: '#e5e7eb', padding: '5px', borderRadius: '12px' }}>
              <button onClick={() => setOrderType('in_hall')} style={{ flex: 1, padding: '10px 5px', borderRadius: '8px', border: 'none', backgroundColor: orderType === 'in_hall' ? '#111827' : 'transparent', color: orderType === 'in_hall' ? '#fff' : '#4b5563', fontWeight: 'bold', cursor: 'pointer', transition: '0.2s', fontSize: '13px' }}>В зале</button>
              <button onClick={() => setOrderType('takeaway')} style={{ flex: 1, padding: '10px 5px', borderRadius: '8px', border: 'none', backgroundColor: orderType === 'takeaway' ? '#111827' : 'transparent', color: orderType === 'takeaway' ? '#fff' : '#4b5563', fontWeight: 'bold', cursor: 'pointer', transition: '0.2s', fontSize: '13px' }}>Навынос</button>
              <button onClick={() => setOrderType('delivery')} style={{ flex: 1, padding: '10px 5px', borderRadius: '8px', border: 'none', backgroundColor: orderType === 'delivery' ? '#111827' : 'transparent', color: orderType === 'delivery' ? '#fff' : '#4b5563', fontWeight: 'bold', cursor: 'pointer', transition: '0.2s', fontSize: '13px' }}>Доставка</button>
            </div>
            {orderType === 'delivery' && (
              <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '20px', marginBottom: '20px', border: '2px solid #ea580c', boxSizing: 'border-box' }}>
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px'}}>
                   <h3 style={{ margin: 0, color: '#111827', fontSize: '16px' }}>📍 Куда доставить?</h3>
                   <button onClick={handleGetLocation} style={{background: '#e0f2fe', color: '#0369a1', border: 'none', padding: '8px 12px', borderRadius: '8px', fontWeight: 'bold', fontSize: '12px', cursor: 'pointer'}}>🗺️ По геоданным</button>
                </div>
                <input type="text" placeholder="Улица или ссылка с карты *" value={address.street} onChange={e=>setAddress({...address, street: e.target.value})} style={{ width: '100%', padding: '14px', borderRadius: '10px', border: '1px solid #d1d5db', marginBottom: '10px', fontSize: '14px', color: '#111827', boxSizing: 'border-box', background: '#f9fafb' }}/>
                <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}><input type="text" placeholder="Дом *" value={address.house} onChange={e=>setAddress({...address, house: e.target.value})} style={{ flex: 1, minWidth: '0', padding: '14px', borderRadius: '10px', border: '1px solid #d1d5db', fontSize: '14px', color: '#111827', boxSizing: 'border-box', background: '#f9fafb' }}/><input type="text" placeholder="Квартира" value={address.apt} onChange={e=>setAddress({...address, apt: e.target.value})} style={{ flex: 1, minWidth: '0', padding: '14px', borderRadius: '10px', border: '1px solid #d1d5db', fontSize: '14px', color: '#111827', boxSizing: 'border-box', background: '#f9fafb' }}/></div>
                <input type="text" placeholder="Комментарий курьеру" value={address.comment} onChange={e=>setAddress({...address, comment: e.target.value})} style={{ width: '100%', padding: '14px', borderRadius: '10px', border: '1px solid #d1d5db', fontSize: '14px', color: '#111827', boxSizing: 'border-box', background: '#f9fafb' }}/>
              </div>
            )}
            <div style={{backgroundColor: '#fff', borderRadius: '20px', padding: '15px', boxSizing: 'border-box'}}>
              {cartItemsArray.length === 0 ? <p style={{textAlign: 'center', color: '#6b7280'}}>Корзина пуста</p> : cartItemsArray.map(item => (
          ...
          <div className="page-container">
            {currentUser.isAnonymous ? (
              <div style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '20px', textAlign: 'center', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
                <div style={{ fontSize: '50px', marginBottom: '10px' }}>🎁</div><h2 style={{ margin: '0 0 10px 0', color: '#111827', fontSize: '22px' }}>Вы не авторизованы</h2><p style={{ color: '#6b7280', marginBottom: '20px', fontSize: '14px', lineHeight: '1.4' }}>Войдите в систему или создайте карту лояльности за 10 секунд, чтобы получать кэшбек с каждого заказа и бронировать столики.</p>
                <button onClick={logout} style={{ width: '100%', padding: '16px', borderRadius: '14px', border: 'none', background: '#ea580c', color: '#fff', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer' }}>Войти / Создать карту</button>
              </div>
            ) : (
              <>
                <div style={{ backgroundColor: '#111827', padding: '25px', borderRadius: '20px', color: '#fff' }}>
                  <h2 style={{ margin: '0 0 10px 0', color: '#fff' }}>{currentUser.name}</h2><p style={{ color: '#10b981', fontSize: '24px', fontWeight: '900', margin: 0 }}>Кэшбек баланс: {availableBonuses} ₸</p>
                </div>
                <button onClick={logout} style={{ width: '100%', padding: '14px', borderRadius: '12px', border: '1px solid #d1d5db', background: '#fff', color: '#ef4444', fontWeight: 'bold', marginTop: '20px', cursor: 'pointer' }}>Выйти из профиля</button>
              </>
            )}
          </div>
        )}
      ...
      <nav className="mobile-nav">
        <button className={`nav-item ${activeGuestTab === 'menu' ? 'active' : ''}`} onClick={() => setActiveGuestTab('menu')}><span className="nav-icon">🍔</span> Меню</button>
        <button className={`nav-item ${activeGuestTab === 'table' ? 'active' : ''}`} onClick={() => setActiveGuestTab('table')}><span className="nav-icon">🪑</span> Залы</button>
        ...
        <button className={`nav-item ${activeGuestTab === 'profile' ? 'active' : ''}`} onClick={() => setActiveGuestTab('profile')}><span className="nav-icon">👤</span> Профиль</button>
      </nav>
    </div>
  );
}